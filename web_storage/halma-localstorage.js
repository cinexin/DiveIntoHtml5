...
/* these two functions "saveGameState" and "resumeGame" uses "localStorage"
	HTML5 object to remember your last movements...*/
...
function resumeGame() {
	...
	/* NOTE THAT all data stored in the local storage is saved
		as a STRING, so we have to parse it to our expected data type in each case...*/
    if (!supportsLocalStorage()) { return false; }
...
function initGame(canvasElement, moveCountElement) {
    ...
    /* we call it to "resumeGame" function to check if we have
    to remember the last user movements...*/
    if (!resumeGame()) {
		newGame();
    }
}
